<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="text-3xl font-bold text-foreground mb-2">Employee Management</h1>
        <p class="text-muted-foreground">Manage your team members and their information</p>
      </div>
      <Button as="a" href="/admin" variant="outline">
      ...
      <CardContent>
        ...
        <div v-if="loading" class="flex items-center justify-center py-12">
          <div class="flex items-center space-x-3">
            ...
            <span class="text-muted-foreground">Loading employees...</span>
          </div>
        </div>
        ...
        <div v-else-if="employees.length === 0" class="flex flex-col items-center justify-center py-12 space-y-4">
          ...
          <div class="text-center">
            <h3 class="text-lg font-semibold text-foreground mb-1">No Employees Found</h3>
            <p class="text-muted-foreground">Add your first employee using the form above.</p>
          </div>
        </div>
    ...
    <Dialog v-model:open="showDeleteModal">
      <DialogContent class="sm:max-w-[425px]">
        ...
        <div class="flex items-center p-4 mb-4 text-sm text-yellow-800 border border-yellow-300 rounded-lg bg-yellow-50 dark:bg-gray-800 dark:text-yellow-300 dark:border-yellow-800">
          ...
          <div>
            <span class="font-medium">Warning!</span> This will permanently delete the employee record.
          </div>
        </div>
        <DialogFooter>
          <Button
...
const onCreateSubmit = handleCreateSubmit(async (values) => {
  ...
  try {
    ...
    resetCreateForm()
    showSuccess('Employee created', `${created.name} has been added successfully`)
    await fetchEmployees()
  } catch (err: any) {
    console.error('Create error:', err)
    showError('Error', err.message || 'Failed to create employee')
  } finally {
    loading.value = false
  }
})
...
const onEditSubmit = handleEditSubmit(async (values) => {
  ...
  try {
    ...
    closeEditModal()
    showSuccess('Employee updated', `${updated.name} has been updated successfully`)
    await fetchEmployees()
  } catch (err: any) {
    console.error('Update error:', err)
    showError('Error', err.message || 'Failed to update employee')
  } finally {
    loading.value = false
  }
})
...
const confirmDelete = async () => {
  ...
  try {
    ...
    closeDeleteModal()
    showSuccess('Employee deleted', `${deletedName} has been removed successfully`)
    await fetchEmployees()
  } catch (err: any) {
    showError('Error', err.message || 'Failed to delete employee')
  } finally {
    loading.value = false
  }
}
